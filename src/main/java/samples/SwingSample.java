 ...
 */
public final class SwingSample {
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            UiMode currentMode = UiMode.SYSTEM_DEFAULT;
            rbSystem.setSelected(true);

            // Apply initial dark/light theme
            applyTheme(currentMode, frame.getContentPane());

            // Change Theme when user toggles
            rbSystem.addActionListener(e -> applyTheme(UiMode.SYSTEM_DEFAULT, frame));
            rbLight.addActionListener(e -> applyTheme(UiMode.LIGHT, frame));
            rbDark.addActionListener(e -> applyTheme(UiMode.DARK, frame));

            // Change Theme when system setting changes
            DETECTOR.addListener(dark -> {
                if (rbSystem.isSelected()) {
                    applyTheme(dark ? UiMode.DARK : UiMode.LIGHT, frame);
                }
            });
    ...
    private static void applyTheme(UiMode mode, Container root) {
